...
@pytest.mark.skipif(
    True, reason="Integration test - run manually or move to regression_tests/"
)
def test_conversion_accuracy():
    ...
    try:
        ...
        if len(expert_params_orig) == 0:
            print(
                "   ⚠️ No _blocks/_scales found - original model might be in dequantized format too!"
            )
        ...
        for name, param in expert_params_dequant.items():
            if "gate_up_proj" in name:
                ...
                print(
                    f"   {name} -> blocks: {expected_blocks_shape}, scales: {expected_scales_shape}"
                )
        ...
        try:
            ...
            original_model_dir = snapshot_download(
                model_id, allow_patterns="*.safetensors"
            )
            original_safetensor_files = list(
                Path(original_model_dir).glob("*.safetensors")
            )

            print(
                f"   📂 Original safetensor files: {[f.name for f in original_safetensor_files]}"
            )
            ...
            if orig_layers == conv_layers:
                ...
                print(
                    "     ❌ Layer sets don't match - this explains the parameter mismatch!"
                )
            ...
            for orig_name, orig_tensor in orig_expert_params.items():
                if orig_name in conv_expert_params:
                    ...
                    if orig_tensor.shape != conv_tensor.shape:
                        print(
                            f"     ❌ Shape mismatch {orig_name}: {orig_tensor.shape} vs {conv_tensor.shape}"
                        )
                        mismatches += 1
                        continue
                    ...
                    if orig_tensor.dtype != conv_tensor.dtype:
                        print(
                            f"     ❌ Dtype mismatch {orig_name}: {orig_tensor.dtype} vs {conv_tensor.dtype}"
                        )
                        mismatches += 1
                        continue
                    ...
                    if torch.equal(orig_tensor, conv_tensor):
                        ...
                        if (
                            diff_count > 0 and diff_count < 20
                        ):  # Only if manageable number
                            ...
                            for i, pos in enumerate(diff_positions[:3]):  # Show first 3
                                ...
                                print(
                                    f"         Position {pos_tuple}: {orig_val} vs {conv_val}"
                                )
            ...
            if total_comparisons > 0:
                print(
                    f"     📊 Match rate: {matches / max(1, matches + mismatches) * 100:.1f}%"
                )
            ...
            try:
                ...
                print(
                    f"   ❌ Failed to load Mini Trainer converted model for inference: {load_e}"
                )
