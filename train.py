...
def save_model(fsdp_model, samples_seen, output_dir, model_name_or_path):
    ...
    state_dict = get_model_state_dict(
        fsdp_model, options=StateDictOptions(full_state_dict=True)
    )
    ...
    if rank == 0:
        ...
        log_rank_0(
            f"\033[1;38;2;0;255;255mSaved model at\033[0m {samples_seen} samples in {time.time() - start:.2f} seconds"
        )
...
def main(
    ...
    max_tokens_per_gpu: int = Option(
        10000, help="Maximum tokens per GPU per minibatch"
    ),
    learning_rate: float = Option(5e-6, help="Peak learning rate"),
    num_warmup_steps: int = Option(
        10, help="Number of warmup steps for the LR scheduler"
    ),
    lr_scheduler: str = Option(
        "constant_with_warmup", help="Learning rate scheduler type"
    ),
    ...
    orthogonal_subspace_learning: bool = Option(
        False, help="Enable SVD based orthogonal subspace training"
    ),
    output_dir: str = Option(
        ..., help="Directory to save checkpoints and logs (required)"
    ),
