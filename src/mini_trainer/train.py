...
def train(
    ...
    Args:
        ...
        params_dict (dict | None, optional): Parameters dictionary to update with final validation loss. Defaults to None.
    ...
    if params_dict is not None and last_validation_loss is not None and is_main_process:
        params_dict["validation_loss"] = last_validation_loss
        # Save updated params to file
        params_path = Path(output_dir) / "training_params.json"
        with open(params_path, 'w') as f:
            json.dump(params_dict, f, indent=4)
        log_rank_0(f"Updated training parameters with final validation loss: {last_validation_loss:.6f}")
        
        # Update wandb config if wandb is being used
        if use_wandb:
            try:
                import wandb
...
def main(
    ...
    train(
        ...
        validation_frequency=validation_frequency,
        params_dict=params if rank == 0 else None,
    )
